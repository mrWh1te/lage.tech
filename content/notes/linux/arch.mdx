...
### Wifi
Arch does not include a DHCP client in the bare-install. I ended up having most success with [dhcpcd](https://wiki.archlinux.org/index.php/Dhcpcd) to get an IP. After determining my wireless card's chipset, I was able to find a driver that supported it. Also, it's possible to find the driver through the hardware's device-id. After the driver was installed, the wifi interface showed up on [ip link](https://wiki.archlinux.org/index.php/Network_configuration#Network_interfaces). I was able to form a wireless connection to my home Wifi network with encryption using [iwctl](https://wiki.archlinux.org/index.php/Iwd#iwctl).
